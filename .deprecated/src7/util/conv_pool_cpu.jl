...
function _conv2_gemm(x0::Array{T,2}, w::Array{T,2}; pad=0, stride=1, xcorr=false) where {T}
    if pad > 0
...
function _conv2(x::Array{T,2}, w::Array{T,2}; pad=0, stride=1, xcorr=false) where {T}
...
function cudnnConvolutionForward(x::Array{T,4}, w::Array{T,4}, y::Array{T,4}; padding=0, stride=1, 
                                    upscale=1, mode=CUDNN_CONVOLUTION, cd=nothing,
                                    algorithm=CUDNN_CONVOLUTION_FWD_ALGO_IMPLICIT_GEMM,
                                    workSpace=0, workSpaceSizeInBytes=0, alpha=1, beta=1,im2col=1) where {T}
    # x: (W,H,C,N)
    # w: (W,H,C,K) 
    # y: (W,H,K,N) 
    fill!(y,0)
...
function cudnnConvolutionBackwardFilter(x::Array{T,4}, dy::Array{T,4}, dw::Array{T,4}; padding=0, stride=1, upscale=1, mode=CUDNN_CONVOLUTION) where {T}
    # x:    (Wx,Hx,Cx,N)
    # dy:   (Wy,Hy,K,N) 
    # dw:    (Ww,Hw,Cw,K) 
    fill!(dw,0)
...
# dx = xcorr(dy, w, 'full')
function cudnnConvolutionBackwardData(w::Array{T,4}, dy::Array{T,4}, dx::Array{T,4}; padding=0, stride=1, upscale=1, mode=CUDNN_CONVOLUTION) where {T}
    fill!(dx,0)
    @assert (padding==0&& stride==1&& upscale==1&& mode==CUDNN_CONVOLUTION)
    Wy,Hy,Ky,N = size(dy)
    Ww,Hw,C,Kw = size(w)
...
function cudnnPoolingForward(x::Array{T,4}, y; window=2, padding=0, stride=window, mode=CUDNN_POOLING_MAX) where {T}
    fill!(y,0)
...
function cudnnPoolingBackward(y::Array{T,4}, dy::Array{T,4}, x::Array{T,4}, dx::Array{T,4}; window=2, padding=0, stride=1, mode=CUDNN_POOLING_MAX) where {T}
    fill!(dx,0)
...
function cudnnGetConvolutionNdForwardOutputDim(x::Array{T,4}, w::Array{T,4}; padding=padding,stride=stride) where {T}
    Wx,Hx,Cx,N = size(x)
    Ww,Hw,Cw,K = size(w)
    @assert Cx==Cw
...
function cudnnGetPoolingNdForwardOutputDim(x::Array{T,4}; window=2, padding=0, stride=1, mode=CUDNN_POOLING_MAX) where {T}
