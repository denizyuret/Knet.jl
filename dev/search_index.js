...

{
    ...
    "category": "section",
    "text": "Sometimes when Knet or CUDA libraries are updated or moved, the precompiled binaries get out of sync and you may get errors like:ccall: could not find function xxx in library libknet8.soI recommend the following steps to refresh everything:shell> rm ~/.julia/compiled/v1.1/Knet/*.ji\nshell> cd ~/.julia/packages/Knet/*/deps\nshell> make clean\nshell> julia\njulia> using Pkg; Pkg.build(\"Knet\")This refreshes all precompiled binaries and should typically solve the problem. If problems continue, you can get support from knet-users."
},
...

{
    ...
    "title": "Knet.progress",
    "category": "function",
    "text": "progress(f, itr; steps, seconds)\nprogress(itr; o...) do x; f(x); end\nprogress(itr; o...)\nprogress!(...)\n\nReturn an iterator which acts exactly like itr, but prints a progressbar:\n\n┣█████████████████▎  ┫ [86.83%, 903/1040, 01:36/01:50, 9.42i/s] 3.87835\n\nHere 86.83% is the percentage completed, 903 is the number of iterations completed, 1040 is the total number of iterations. 01:36 is elapsed time, 01:50 is the estimated total time, 9.42i/s is the average number of iterations completed per second. If the speed is less than 1, the average number of seconds per iteration (s/i) is reported instead.  The bar, percent, total iterations, and estimated total time are omitted for iterators whose size is unknown. 3.87835 is the output of f applied to the last value generated by itr.\n\nThe progress bar is updated and f is called with the most recent value of itr every steps iterations or every seconds seconds in addition to the first and the last iteration. If neither steps nor seconds is specified the default is to update every second.\n\nf can be specified by the first two forms above, if not specified (the third form) nothing gets printed at the end of the line.  The last form, progress!(...), is equivalent to (for x in progress(...) end), i.e.  iterates over the object created by progress(...) and returns nothing.\n\n\n\n\n\n"
},
...
{
    "location": "nce/#",
    "page": "Noise Contrastive Estimation",
...
{
    "location": "nce/#Noise-Contrastive-Estimation-1",
    "page": "Noise Contrastive Estimation",
    "title": "Noise Contrastive Estimation",
    "category": "section",
    "text": "(c) Deniz Yuret, 2015-10-09, last updated: 2019-10-01Noise contrastive estimation (NCE) replaces the expensive vocabulary-sized softmax operation at the final layer of language models with a cheaper sampling based operation, which results in significant speed-up during training. To motivate NCE, let us start with the basic equations for probabilistic models.To model the probability distribution p(y) of a set of objects y in mathcalY, we start with a score function s_theta(y)mathcalYrightarrowmathbbR with adjustable weights theta. For example log-linear models use s_theta(y)=theta^T phi(y) where theta is a weight vector and phi(y)mathcalYrightarrowmathbbR^D is a function that maps an element of mathcalY to a vector of real valued features.  Conditional models use p(yx), s_theta(xy), and phi(xy) instead. We will stick with non-conditional notation for brevity.To go from arbitrary valued scores to normalized probability estimates we use exponentiation and normalization (which, for some reason, is called the softmax function):p_theta(y) = fracexp s_theta(y)sum_yin mathcalY exp s_theta(y)The maximum likelihood estimate (MLE) training objective is to maximize the estimated probability of a given training set Y=y_1ldotsy_n. Assuming the instances in the training set are selected independently we have log p_theta(Y) = sum_yin Y log p_theta(y). The contribution of an individual instance to the objective is:L_MLE(theta) = log p_theta(y) = s_theta(y) - logsum_yinmathcalYexp s_theta(y)Stochastic gradient descent (SGD) uses the gradient of this quantity with respect to the weights theta to find the MLE solution:beginalign*\n	nabla L_MLE(theta) = nabla s_theta(y) - nabla logsum_yinmathcalYexp s_theta(y) \n	= nabla s_theta(y) - fracsum_yinmathcalYexp s_theta(y) nabla s_theta(y)sum_yinmathcalYexp s_theta(y) \n	= nabla s_theta(y) - sum_yinmathcalY p_theta(y) nabla s_theta(y) \nendalign*Note that the second term in the final equation involves a sum over the whole mathcalY which is typically a computational nightmare. In order to avoid this sum, people have come up with all sorts of tricks.  One simple example is the perceptron approximation:logsum_yinmathcalYexp s_theta(y) approx max_yinmathcalY s_theta(y) \nnabla L(theta) approx nabla s_theta(y) - nabla max_yinmathcalY s_theta(y)Here is an example to demonstrate why this makes sense. Let\'s say mathcalY has three elements and their scores, s_theta(y), are 10, 20, and 30. When we exponentiate these scores we get e^10, e^20 and e^30. Note that even though 20 and 30 are not all that different, e^30approx 10^13 is significantly larger than e^20approx 5times 10^9. When we add the exponentials, e^10+e^20+e^30, the result will not be significantly different from e^30, and thus the result of the final log will not be that different from 30 (it is 30.000045401... if you are curious) (and that is why logsumexp should be called the softmax function).The perceptron approximation seems to replace the expensive summation with an equally expensive looking max operation. Fortunately the max operation can be performed fast for certain classes of problems. This will be the topic of another chapter, for now let\'s get back to NCE.NCE takes a different approach to avoid the costly summation. Instead of modeling the empirical distribution p(y) directly, it proposes solving the related binary classification problem of distinguishing samples generated by p(y) from samples generated by a \"noise\" distribution q(y). It is common practice to use a simple uniform distribution or, for language models, the unigram distribution for q(y).How is the binary classification problem of deciding p vs q related to the original density estimation problem of modeling p? Assume we generate k noise samples y_1ldots ksim q(y) for each real sample and add them to our original training data. We label each real sample with d=1 and each noise sample with d=0 to train a binary classifier. The joint probability of the samples and labels in this new dataset is:p(d=1 y) = frac1k+1p(y) qquad p(d=0 y) = frackk+1q(y)The conditional probability of the label given the sample is:p(d=1mid y) = fracp(y)p(y)+kq(y) qquad p(d=0mid y) = frackq(y)p(y)+kq(y)This means p(d=1mid y) and p(y) are related by a simple algebraic identity. In fact, if somebody hands us a good estimate for p(d=1mid y), we can turn it into an esimate for p(y):p(y) = k q(y) fracp(d=1mid y)p(d=0mid y)NCE suggests training the following binary classifier model for p(d=1mid y) on the dataset with noise samples.p_theta(d=1mid y) = fracexp s_theta(y)exp s_theta(y) + kq(y)Using this model will give us the following for p_theta(y):beginalign*\n	p_theta(y) = k q(y) fracp_theta(d=1mid y)p_theta(d=0mid y) \n	= kq(y) fracexp s_theta(y)kq(y) \n	= exp s_theta(y)\nendalign*which amounts to assuming our costly normalization term Z=sum_yin mathcalY exp s_theta(y) is 1.Our new objective is to maximize the conditional probability of the NCE dataset. Consider the conditional log probability of a real sample y_0 and k noise samples y_1ldots y_k:beginalign* L_NCE(theta) \n	= log p_theta(d=1mid y_0) + sum_i=1^k log p_theta(d=0mid y_i) \n	= s_theta(y_0) - log(exp s_theta(y_0) + kq(y_0)) + sum_i=1^k log(kq(y_i)) - log(exp s_theta(y_i) + kq(y_i)) \nendalign*The gradient of the new objective is:beginalign* nabla L_NCE(theta)\n	= nabla s_theta(y_0) - sum_i=0^k nabla log(exp s_theta(y_i) + kq(y_i)) \n	= nabla s_theta(y_0) - sum_i=0^k p_theta(d=1mid y_i) nabla s_theta(y_i) \nendalign*In the limit krightarrowinfty we see that the NCE gradient approaches the MLE gradient:beginalign* nabla L_NCE(theta)\n	rightarrow nabla s_theta(y_0) - sum_yinmathcalY k q(y) p_theta(d=1mid y) nabla s_theta(y) \n	= nabla s_theta(y_0) - sum_yinmathcalY k q(y) fracexp s_theta(y)exp s_theta(y) + kq(y) nabla s_theta(y) \n	rightarrow nabla s_theta(y_0) - sum_yinmathcalY exp s_theta(y) nabla s_theta(y) \n	= nabla s_theta(y_0) - sum_yinmathcalY p_theta(y) nabla s_theta(y)\nendalign*What does this all mean computationally? Let\'s compare the operations of MLE and NCE language models in their final layers. Say both models use a D-dimensional internal representation. For the MLE model, the output is a V-dimensional probability vector where V is the vocabulary size. The forward pass involves multiplication of the D-dimensional internal representation with a V times D decoding matrix and normalization of the result, an O(VD) operation.The NCE model, on the other hand, only needs the scores of the correct word and K additional noise sample words during training. This involves extracting K+1 rows from the V times D decoding matrix, multiplying the D-dimensional internal representation with the resulting (K+1) times D matrix and no normalization, an O(KD) operation. Because K ll V this results in a large speed-up.Here is the backward pass for MLE (with subscripts dropped for brevity):beginalign*\n	p(y) = fracexp s(y)sum_yinmathcalYexp s(y) \n	L = log p(y) = s(y) - logsum_yinmathcalYexp s(y) \n	partial Lpartial s(y) = 1 - p(y) \n	partial Lpartial s(y) = -p(y) \nendalign*Here is the backward pass for NCE:beginalign*\n	p(d=1mid y) = fracexp s(y)exp s(y) + kq(y) qquad p(d=0mid y) = frackq(y)exp s(y) + kq(y) \n	L = log p(d=1mid y) + sum_i=1^k log p(d=0mid y_i) \n	L = s(y) - log(exp s(y)+kq(y)) + sum_i=1^k log(kq(y_i)) - log(exp s(y_i)+kq(y_i)) \n	partial Lpartial s(y) = 1 - p(d=1mid y) \n	partial Lpartial s(y_i) = -p(d=1mid y_i) \nendalign*Research Ideas:Can we use the words in a minibatch as noise samples for each other, presumably with an importance sampling correction factor?\nCan we use other models for the binary classification problem?References:Gutmann, M. U., & Hyvärinen, A. (2012). Noise-contrastive estimation of unnormalized statistical models, with applications to natural image statistics. The Journal of Machine Learning Research, 13(1), 307-361.Mnih, A., & Teh, Y. W. (2012). A fast and simple algorithm for training neural probabilistic language models. arXiv preprint arXiv:1206.6426.Dyer, C. (2014). Notes on Noise Contrastive Estimation and Negative Sampling. arXiv preprint arXiv:1410.8251."
},
