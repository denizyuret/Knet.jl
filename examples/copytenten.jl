...
function copytenten(args=ARGS)
    ...
    seed > 0 && setseed(seed)
    global data = S2SData(`tenten.pl -l $maxlen`; batchsize=batchsize, ftype=eval(parse(ftype)), dense=dense, dict=dictfile)
    ...
    @time train(model, data, softloss; gclip=gclip, maxnorm=maxnorm, losscnt=losscnt, lossreport=lossreport)
