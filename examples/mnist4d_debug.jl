...
function main(args=ARGS)
    ...
    global x1 = reshape(MNIST.xtrn[:,:,:,1], 28,28,1,1)
    global y1 = reshape(MNIST.ytrn[:,1], 10,1)

    global net1 = compile(:lenet_model)
    ypred = forw(net1, x1)
    global net2 = Knet.cpucopy(net1)
    @show to_host(forw(net1,x1))
    @show forw(net2,x1)

    #=
    setp(net1; lr=lr)
    setp(net2; lr=lr)
    @show back(net, CudaArray(y1), softloss; getdx=true)
    @show back(net2, y1, softloss; getdx=true)
    =#

    # back(net2, y1, zeroone)
    #=
    # global dtrn = minibatch(MNIST.xtrn, MNIST.ytrn[:,:,:,1], batchsize)
    # global dtst = minibatch(MNIST.xtst[:,:,:,1], MNIST.ytst[:,:,:,1], batchsize)

    global net = compile(:lenet_model)
    global net2 = Knet.cpucopy(net)
    setp(net; lr=lr)
    setp(net2; lr=lr)

    @show test(net,dtst,zeroone)
    @show test(net2,dtst,zeroone)
