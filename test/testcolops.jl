...
iseq(a,b)=(convert(Array,a)==convert(Array,b))
...
for B in (
    ...
    for A in (
        ...
        # CSLICE
        @test iseq(cslice!(copy(a), b, 3:8), b0[:,3:8])

        # CCAT
        r = rand(1:ccount(b), 10)
        @test iseq(ccat!(copy(a), b, r), hcat(a0, b0[:,r]))

        # CCOPY
        if !issparse(b)
            bb = ccopy!(copy(b), 5, a) # copies a into b starting at b column 5
            @test iseq(bb, hcat(b0[:,1:4], a0, b0[:,25:end]))
            
            # CADD
            if atype(a) == atype(b)
                cc = cadd!(copy(b), 5, a)  # adds a into b starting at b column 5
                @test iseq(cc, hcat(b0[:,1:4], (a0+b0[:,5:24]), b0[:,25:end]))
            end
        end
