...
@testset "bmm" begin
    ...
    At = KnetArray(rand(4,3,10))
    B = KnetArray(rand(4,5,10))
    Bt = KnetArray(rand(5,4,10))
    C = KnetArray(rand(3,5,10))
    @test bmm!('N','N',1.0,A,B,0.0,C) === C
    @test bmm!('N','T',1.0,A,Bt,0.0,C) === C
    @test bmm!('T','N',1.0,At,B,0.0,C) === C
    @test bmm!('T','T',1.0,At,Bt,0.0,C) === C
end #end of testset
