...
@testset "layers21/conv" begin
    ...
    function convtest(
        ...
        z = usez ? randn!(conv(w,x;activation,alpha,beta,channelmajor,crosscorrelation,dilation,group,padding,stride)) : nothing
        ...
        qx,qw,qb,qz = (i->i===nothing ? nothing : Param(i)).((x,w,b,z))
        c = Conv(pw; bias=pb, padding, stride, dilation, group, crosscorrelation, channelmajor, activation, alpha, beta)
        if isa(aw, Array) || gcheck_only
            r1 = true
        else
            ccpu = Conv(qw; bias=qb, padding, stride, dilation, group, crosscorrelation, channelmajor, activation, alpha, beta)
            r1 = isapprox(Array(c(px, pz)), ccpu(qx, qz))
        end
        r2 = @gcheck c(px, pz)
        r1 && r2
    end
