...
function rosenopt(w, params; verbose=false, ftol = 1e-1, xtol = 1e-10, maxiter = 10000)
    i = 1
    prev = 0
    current = 1
    t0 = time()
    ...
    t1 = time()
    if verbose
        @printf("%s: f=%f Δ=%f iter=%-5d time=%.2f type=%s opt=%s\n",
                (current <= ftol ? "PASS" : "FAIL"),
                current, prev-current, i-1, t1-t0, typeof(w), typeof(params))
    end
    return current <= ftol
end
...
@testset "update!" begin
    ...
    @test rosenopt(copy(w),Adagrad(lr=0.4, eps=1e-6))
    @test rosenopt(copy(w),Adadelta(lr=0.4, rho=0.5, eps=1e-6))
    @test rosenopt(copy(w),Rmsprop(lr=0.005, rho=0.9, eps=1e-6))
