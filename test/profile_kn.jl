...
tofloat{T<:AbstractFloat}(a::CudaArray{T})=a
_dbg{T}(x::CudaArray{T})=Symbol("C$(join([id2(x),size(x)...,8*sizeof(T)],'_'))")
...
for (f,g) in broadcast2arg
    ...
    @eval @primitive $f(x1::Number,x2::CudaArray)::y  unbroadcast(y,x1,dy->dy.*$(g[1]))  unbroadcast(y,x2,dy->dy.*$(g[2]))
    @eval @primitive $f(x1::CudaArray,x2::Number)::y  unbroadcast(y,x1,dy->dy.*$(g[1]))  unbroadcast(y,x2,dy->dy.*$(g[2]))
end
for (f,g) in math2arg
    @eval @primitive $f(x1::CudaArray,x2::CudaArray)::y  unbroadcast(y,x1,dy->dy.*$(g[1]))  unbroadcast(y,x2,dy->dy.*$(g[2]))
    @eval @primitive $f(x1::Number,x2::CudaArray)::y  unbroadcast(y,x1,dy->dy.*$(g[1]))  unbroadcast(y,x2,dy->dy.*$(g[2]))
...
for (f,g) in float2arg
    @eval @primitive $f(x1::CudaArray,x2::CudaArray)::y unbroadcast(y,x1,$(g[1])) unbroadcast(y,x2,$(g[2]))
    @eval @primitive $f(x1::Number,x2::CudaArray)::y unbroadcast(y,x1,$(g[1])) unbroadcast(y,x2,$(g[2]))
    @eval @primitive $f(x1::CudaArray,x2::Number)::y unbroadcast(y,x1,$(g[1])) unbroadcast(y,x2,$(g[2]))
end
for f in broadcast2cmp
    @eval begin
        @zerograd $f(x1::CudaArray,x2::CudaArray)
        @zerograd $f(x1::CudaArray,x2::Number)
        @zerograd $f(x1::Number,x2::CudaArray)
    end
end

Base.eltype{T}(x::CudaArray{T})=T
@primitive  sum(x::CudaArray)  (dy->convert(eltype(x),dy).+zeros(x))
@primitive  sum(x::CudaArray,i...)  (dy->dy.+zeros(x))
Base.size(x::CudaArray,i)=(if i>ndims(x); 1; else; size(x)[i]; end)

include("profile.jl")
d0kn = map(i->map(CudaArray,i),d0)
# w1kn = map(CudaArray,w1)
w2kn = map(CudaArray,w2)
