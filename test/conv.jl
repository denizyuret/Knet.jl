...
@testset "conv" begin
    if gpu() >= 0
        ...
        #@test_skip gradcheck(unpool, x32)
        ...
        #@test_skip gradcheck(unpool, x; kwargs=[:window=>3])
        # padding=1 (default=0)
        @test gradcheck(pool, x; kwargs=[:padding=>1])
        #@test_skip gradcheck(unpool, x; kwargs=[:padding=>1])
        ...
        #@test_skip gradcheck(unpool, x; kwargs=[:stride=>3])
        ...
        #@test_skip gradcheck(unpool, x; kwargs=[:mode=>1])
        ...
        #@test_skip gradcheck(unpool, x; kwargs=[:mode=>2])
        # alpha=2 (default=1)
        #@test_skip gradcheck(pool, x; kwargs=[:alpha=>2])
        #@test_skip gradcheck(unpool, x; kwargs=[:alpha=>2])
