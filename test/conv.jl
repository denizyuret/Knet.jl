...
Base.isapprox(k::KnetArray,a::Array)=isapprox(Array(k),a)
ax = rand41(5,4,3,2)
aw = rand41(3,3,3,3)
...
@testset "cpuconv" begin
    ...
    ### padding=1 (default=0)
    #FAIL @test gradcheck(pool, ax; kwargs=[:padding=>1])
    #FAIL @test gradcheck(unpool, ax; kwargs=[:padding=>1])
    @test gradcheck(conv41, (aw,ax); rtol=0.05, kwargs=[:padding=>1])
    @test gradcheck(deconv41, (aw,ax); rtol=0.05, kwargs=[:padding=>1])
    #FAIL @test gradcheck(pool, ax; kwargs=[:padding=>(1,1)])
    #FAIL @test gradcheck(unpool, ax; kwargs=[:padding=>(1,1)])
    ...
    #FAIL @test gradcheck(unpool, ax; kwargs=[:stride=>3])
    ...
    #FAIL @test gradcheck(unpool, ax; kwargs=[:stride=>(3,3)])
    ...
    #FAIL @test gradcheck(pool, ax; kwargs=[:mode=>2])
    #FAIL @test gradcheck(unpool, ax; kwargs=[:mode=>2])

    ### alpha=2 (default=1)
    @test gradcheck(pool, ax; kwargs=[:alpha=>2])
    #FAIL @test gradcheck(unpool, ax; kwargs=[:alpha=>2])
...
if gpu() >= 0; @testset "gpuconv" begin
    ...
    #FAIL @test gradcheck(unpool, kx5)
    ...
    ### padding=1 (default=0)
    #FAIL @test isapprox(pool(kx;padding=1), pool(ax;padding=1))
    @test gradcheck(pool, kx; kwargs=[:padding=>1])
    #FAIL @test isapprox(unpool(kx;padding=1), unpool(ax;padding=1))
    #FAIL @test gradcheck(unpool, kx; kwargs=[:padding=>1])
    ...
    #FAIL @test isapprox(pool(kx;padding=(1,1)), pool(ax;padding=(1,1)))
    @test gradcheck(pool, kx; kwargs=[:padding=>(1,1)])
    #FAIL @test isapprox(unpool(kx;padding=(1,1)), unpool(ax;padding=(1,1)))
    #FAIL @test gradcheck(unpool, kx; kwargs=[:padding=>(1,1)])
    ...
    #FAIL @test gradcheck(unpool, kx; kwargs=[:stride=>3])
    @test isapprox(conv4(kw,kx;stride=3), conv4(aw,ax;stride=3))
    @test gradcheck(conv41, (kw,kx); rtol=0.05, kwargs=[:stride=>3])
    #FAIL @test isapprox(deconv4(kw,kx;stride=3), deconv4(aw,ax;stride=3))
    ...
    #FAIL @test gradcheck(unpool, kx; kwargs=[:stride=>(3,3)])
    @test isapprox(conv4(kw,kx;stride=(3,3)), conv4(aw,ax;stride=(3,3)))
    @test gradcheck(conv41, (kw,kx); rtol=0.05, kwargs=[:stride=>(3,3)])
    #FAIL @test isapprox(deconv4(kw,kx;stride=(3,3)), deconv4(aw,ax;stride=(3,3)))
    ...
    ### mode=2 (only for pool)
    #FAIL @test isapprox(pool(kx;mode=2), pool(ax;mode=2))
    @test gradcheck(pool, kx; kwargs=[:mode=>2])
    #FAIL @test isapprox(unpool(kx;mode=2), unpool(ax;mode=2))
    #FAIL @test gradcheck(unpool, kx; kwargs=[:mode=>2])

    ### alpha=2 (default=1)
    @test isapprox(pool(kx;alpha=2), pool(ax;alpha=2))
    #FAIL @test gradcheck(pool, kx; kwargs=[:alpha=>2])
    @test isapprox(unpool(kx;alpha=2), unpool(ax;alpha=2))
    #FAIL @test gradcheck(unpool, kx; kwargs=[:alpha=>2])
