...
# startin with Julia 0.7 we need constructors instead of convert # rene
KnetArray(x::KnetArray{T,N}) where {T,N} = x
KnetArray{T}(x::KnetArray{T,N}) where {T,N} = x
KnetArray{T,N}(x::KnetArray{T,N}) where {T,N} = x
KnetArray{T}(x::KnetArray{S,N}) where {T,N,S} = KnetArray{T,N}(x)
KnetArray{T,N}(x::KnetArray{S,N}) where {T,N,S} = KnetArray{T,N}(unsafe_copy!(Array{S}(size(x)), 1, x, 1, length(x)))
...
# startin with Julia 0.7 we need constructors instead of convert # rene
KnetArray(x::AbstractArray{T,N}) where {T,N} = KnetArray{T,N}(x)
KnetArray{T}(x::AbstractArray{S,N}) where {T,N,S} = KnetArray{T,N}(x)
KnetArray{T,N}(x::AbstractArray{S,N}) where {T,N,S} = unsafe_copy!(KnetArray{T}(size(x)), 1, Array{T,N}(x), 1, length(x))
# Array <- KnetArray
Array(x::KnetArray{T,N}) where {T,N} = Array{T,N}(x)
Array{T}(x::KnetArray{S,N}) where {T,N,S} = Array{T,N}(x)
Array{T,N}(x::KnetArray{S,N}) where {T,N,S} = Array{T,N}(unsafe_copy!(Array{S}(size(x)), 1, x, 1, length(x)))
...
Base.deepcopy_internal(x::KnetArray, s::IdType)=if haskey(s,x); s[x]; else; copy(x); end
