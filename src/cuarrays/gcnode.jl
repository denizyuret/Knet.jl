...
function minidx!(q::PriorityQueue{WeakRef,Int,typeof(Base.Order.Reverse)},k::CuArray,v::Int)
...
function gcnode(n::Node, tape::Tape)  ## 16.3μs
    ...
    if n.Value isa Result # && n.outgrad isa KnetArray
        for ptr in cuarrays(n.outgrad); minidx!(gcnode_queue, ptr, ni); end
    end
    @inbounds for i in 1:length(n.parents);  ## 2.43μs
        ...
        if parent.Value isa Result
            ...
            for ptr in cuarrays(parent.outgrad); minidx!(gcnode_queue, ptr, pi); end
        else
