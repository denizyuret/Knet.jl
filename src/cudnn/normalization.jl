...
@primitive1((cudnnNormalizationForwardAutoGrad(x, scale, bias, z; mean, variance, y, mode, normOps, algo, alpha, beta, epsilon, groupCnt, exponentialAverageFactor, savedMean, savedInvVariance, activationDesc, xDesc, yDesc, zDesc, normScaleBiasDesc, normMeanVarDesc, workspace, reserveSpace, dx, dscale, dbias, dz),
             dy, _y),
...
# Only called with tracked arguments: either during training or during inference with Param args.
function forw(f::typeof(cudnnNormalizationForwardAutoGrad), x, scale, bias, z; mean, variance, y, mode, normOps, algo, alpha, beta, epsilon, groupCnt, exponentialAverageFactor, savedMean, savedInvVariance, activationDesc, xDesc, yDesc, zDesc, normScaleBiasDesc, normMeanVarDesc, workspace, reserveSpace, dx, dscale, dbias, dz)
    ...
    if recording()              # we are taking gradients
        ...
        @show workspaceSize
        workspace[] = cudnnTempSpace(10^6) #DBG workspaceSize[])
        ...
        @show reserveSpaceSize
        reserveSpace[] = cudnnTempSpace(10^6) #DBG reserveSpaceSize[])
