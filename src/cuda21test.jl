...
function cuda21test(fname, jname=fname, o...)
    ...
    isapprox(out32,to_host(col32)) || warn("$fname 32 mat col")
    print("gpu row32 ")
    @time cuda21rep(f32,mat32,row32)
    print("cpu row32 ")
    @time out32 = fcpu21rep(fcpu,mat32b,row32b)
    isapprox(out32,to_host(row32)) || warn("$fname 32 mat row")
    ...
    isapprox(out64,to_host(col64)) || warn("$fname 64 mat col")
    print("gpu row64 ")
    @time cuda21rep(f64,mat64,row64)
    print("cpu row64 ")
    @time out64 = fcpu21rep(fcpu,mat64b,row64b)
    isapprox(out64,to_host(row64)) || warn("$fname 64 mat row")
end
