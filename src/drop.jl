...
GPU && (drop(x::CudaArray{Float32}, y::CudaArray{Float32}, xdrop::CudaArray{Float32}, dropout, scale)=ccall((:drop32,libkunet),Void,(Cint,Ptr{Float32},Ptr{Float32},Ptr{Float32},Cdouble,Cdouble),length(x),x,xdrop,dropout,scale))
GPU && (drop(x::CudaArray{Float64}, y::CudaArray{Float64}, xdrop::CudaArray{Float64}, dropout, scale)=ccall((:drop64,libkunet),Void,(Cint,Ptr{Float64},Ptr{Float64},Ptr{Float64},Cdouble,Cdouble),length(x),x,xdrop,dropout,scale))
