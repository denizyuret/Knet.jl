...
relu(w::Mat,b::Vec)=ccall((:relu,libjnet), Layer,
			  (Cint,Cint,Cmat,Cmat), 
			  size(w,1), size(w,2), w, b)

soft(w::Mat,b::Vec)=ccall((:soft,libjnet), Layer,
			  (Cint,Cint,Cmat,Cmat), 
			  size(w,1), size(w,2), w, b)

forward(net::Net, x::Mat, y::Mat, batch::Int)=ccall((:forward,libjnet), Void, 
						    (Cnet, Cmat, Cmat, Cint, Cint, Cint),
						    net, x, y, length(net), size(x,2), batch)
