...
- [Introduction to Knet](tutorial.md#Introduction-to-Knet-1)
    - [Philosophy](tutorial.md#Philosophy-1)
    - [Tutorial](tutorial.md#Tutorial-1)
        - [Linear regression](tutorial.md#Linear-regression-1)
        - [Softmax classification](tutorial.md#Softmax-classification-1)
        - [Multi-layer perceptron](tutorial.md#Multi-layer-perceptron-1)
        - [Convolutional neural network](tutorial.md#Convolutional-neural-network-1)
        - [Recurrent neural network](tutorial.md#Recurrent-neural-network-1)
    - [Benchmarks](tutorial.md#Benchmarks-1)
    - [Under the hood](tutorial.md#Under-the-hood-1)
        - [KnetArrays](tutorial.md#KnetArrays-1)
        - [AutoGrad](tutorial.md#AutoGrad-1)
    - [Contributing](tutorial.md#Contributing-1)
...
Knet provides the [`conv4`](reference.md#Knet.conv4) and [`pool`](reference.md#Knet.pool) functions for the implementation of convolutional nets:
...
Currently convolution and pooling are only supported on the GPU for 4-D and 5-D arrays. So we reshape our data and transfer it to the GPU along with the parameters by converting them into [`KnetArray`](reference.md#Knet.KnetArray)s:
...
At this point we can train the network on any given piece of text (or other discrete sequence). For efficiency it is best to minibatch the training data and run BPTT on small subsequences. See [charlm.jl](https://github.com/denizyuret/Knet.jl/blob/master/examples/charlm.jl) for details. Here is a sample run on 'The Complete Works of William Shakespeare':
...
Knet relies on the [AutoGrad](https://github.com/denizyuret/AutoGrad.jl) package and the [KnetArray](reference.md#KnetArray-1) data type for its functionality and performance. AutoGrad computes the gradient of Julia functions and KnetArray implements high performance GPU arrays with custom memory management. This section briefly describes them.
...
Knet is an open-source project and we are always open to new contributions: bug reports and fixes, feature requests and contributions, new machine learning models and operators, inspiring examples, benchmarking results are all welcome. If you need help or would like to request a feature, please consider joining the [knet-users](https://groups.google.com/forum/#!forum/knet-users) mailing list. If you find a bug, please open a [GitHub issue](https://github.com/denizyuret/Knet.jl/issues). If you would like to contribute to Knet development, check out the [knet-dev](https://groups.google.com/forum/#!forum/knet-dev) mailing list and [Tips for developers](install.md#Tips-for-developers-1).
